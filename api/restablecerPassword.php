    ...
    if (isset($_POST['API_KEY']) && API_KEY == $_POST['API_KEY']) { // si se ha pasado una clave api y es correcta
        if (isset($_POST['codUsuario'])) {
            if (RESTPropio::restablecerPassword($_POST['codUsuario'])) {
                $jsonResponse["respuesta"] = "El password del usuario se ha restablecido con exito con exito";
            } else {
                $jsonResponse['error'] = "No se ha podido restablecer el password del usuario"; // guardamos un mensaje de error en el array
            }
        }else{
