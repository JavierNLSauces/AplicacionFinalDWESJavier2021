...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { // si el metodo de peticion es POST
    if (isset($_POST['API_KEY']) && API_KEY == $_POST['API_KEY']) { // si se ha pasado una clave api y es correcta
        if (isset($_POST['descUsuario'])) {
            $aUsuarios = RESTPropio::obtenerUsuariosPorDescripcion($_POST['descUsuario']);
            if (!empty($aUsuarios)) {
                $jsonResponse["respuesta"] = $aUsuarios;
            } else {
                $jsonResponse['error'] = "No se ha podido obtener los usuarios solicitados"; // guardamos un mensaje de error en el array
            }
        }else{
